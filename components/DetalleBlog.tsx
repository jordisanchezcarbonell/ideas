type Props = {
  promise: Promise<any>;
};
import Link from 'next/link';
import { notFound } from 'next/navigation';

export default async function DettaleBlog({ promise }: Props) {
  const attributesDetalle = await promise;

  if (!attributesDetalle) notFound();
  return (
    <div className="p-4">
      <Link
        href="/"
        type="button"
        className="w-full flex items-center justify-center w-1/2 px-5 py-2 text-sm text-gray-700 transition-colors duration-200 bg-white border rounded-lg gap-x-2 sm:w-auto dark:hover:bg-gray-800 dark:bg-gray-900 hover:bg-gray-100 dark:text-gray-200 dark:border-gray-700 mb-10"
      >
        <svg
          className="w-5 h-5 rtl:rotate-180"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth="1.5"
          stroke="currentColor"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M6.75 15.75L3 12m0 0l3.75-3.75M3 12h18"
          />
        </svg>
        Go back
      </Link>

      {attributesDetalle[0] && attributesDetalle[0].attributes.Content && (
        <div
          className="whitespace-pre-line"
          dangerouslySetInnerHTML={{
            __html: attributesDetalle[0].attributes.Content,
          }}
        />
      )}
    </div>
  );
}
